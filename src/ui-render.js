...
function technicalDecisionLabel(decision){
  return {
    ...
    wait:'等待',
    add_watch:'接近加仓观察',
    add_triggered:'加仓条件触发',
    reduce_watch:'减仓风险提示',
    reduce_triggered:'减仓风险触发',
    hold:'持有复核',
    review:'需要复核'
  }[decision]||'观察';
}
function technicalDecisionText(td){
  return [td&&td.trendStatus,td&&td.technicalSummary,td&&td.actionHint].concat(Array.isArray(td&&td.riskFlags)?td.riskFlags:[]).filter(Boolean).join(' ');
}
...
function calculateTechnicalDecision(stock){
  ...
  const seriousRisk=/breakdown|downtrend|跌破关键|趋势破坏|破位|放量下跌|高位放量杀跌|跌破ma60|跌破60日|跌破120日/.test(riskText);
  const weakRisk=/跌破|压力|动能.*弱|红柱缩短|波动加大|回落|转弱|背离/.test(txt);
  const belowMa20=currentPrice>0&&td.ma20>0&&currentPrice<td.ma20;
  const belowMa60=currentPrice>0&&td.ma60>0&&currentPrice<td.ma60;
  ...
  const triggers=[];
  const risks=[];
  if(trend)reasons.push(`趋势状态：${td.trendStatus}`);
  ...
  if(td.actionHint)reasons.push(`技术提示：${td.actionHint}`);
  ...
  if(weakRisk)risks.push(...normalizeStringArray(td.riskFlags).slice(0,3));
  ...
  if(seriousRisk||belowMa60||trend==='breakdown'||trend==='downtrend'){
    const triggered=(sellTriggered||highPosition||seriousRisk&&belowMa60);
...
function currentActionPanel(stock){
  ...
  const action=tech&&tech.title?tech.title:actionKindLabel(info.text,d.action);
  ...
  const risk=(tech&&tech.riskFlags&&tech.riskFlags.length?tech.riskFlags:[]).concat(allocationWarnings,(d.warnings||[]).slice(0,2),ex.executionWarnings||[]).slice(0,5);
  ...
  const summary=tech&&tech.summary?tech.summary:info.text;
  const matched=tech&&tech.triggerMatched&&tech.triggerMatched.length?tech.triggerMatched:[trigger];
  const reasons=tech&&tech.reason&&tech.reason.length?tech.reason:[info.text];
...
function allocationDecisionContext(stock){
  ...
  return {
    ...
    financialReview:reviews.financialReview||null,
    valuationData:normalizeValuationData(stock.valuationData),
...
function allocationDecisionPromptText(stock){
  ...
  return [
    ...
    '',
    '【分析要求】',
    '请从七个维度进行配置分析：宏观、行业、公司、财报、估值、情绪、技术面。',
...
function allocationDecisionPanel(stock){
  ...
  const hasContent=Boolean(ad.conclusion||ad.recommendedWeightRange||ad.allocationReasons.length||ad.keyRisks.length||ad.suggestedActions.length);
  const dims=ALLOCATION_DIMENSIONS.map(k=>allocationDimensionHtml(k,ad.dimensions[k])).join('');
  return `<div class="card" style="margin-bottom:14px;border-left:4px solid var(--gold)"><div class="card-title">配置决策</div><div class="dash" style="margin:0"><div><div class="card-title">配置结论</div><div class="text" style="max-width:none">${esc(ad.conclusion||'暂无配置决策')}</div><div class="card-note">更新：${esc(ad.updatedAt||'—')} · 置信度 ${esc(ad.confidence||'low')}</div></div><div><div class="card-title">配置区间</div><div class="card-num" style="font-size:20px">${esc(ad.recommendedWeightRange||'—')}</div><div class="card-note">建议目标 ${allocationPercentText(ad.recommendedTargetWeight)} · 最大 ${allocationPercentText(ad.recommendedMaxWeight)}</div></div><div><div class="card-title">角色 / 目标调整</div><div class="card-note">${esc(ad.recommendedRole||'—')} · ${esc(ALLOCATION_TARGET_LABELS[ad.targetAdjustment]||ad.targetAdjustment)}</div></div><div><div class="card-title">新增资金</div><div class="card-note">${esc(ALLOCATION_CAPITAL_LABELS[ad.capitalAllocationView]||ad.capitalAllocationView)}</div></div></div>${hasContent?`<div class="text" style="max-width:none;margin-top:10px"><b>配置理由：</b>${ad.allocationReasons.length?ad.allocationReasons.map(esc).join('；'):'—'}<br><b>核心风险：</b>${ad.keyRisks.length?ad.keyRisks.map(esc).join('；'):'—'}<br><b>建议动作：</b>${ad.suggestedActions.length?ad.suggestedActions.map(esc).join('；'):'—'}${ad.notes?'<br><b>备注：</b>'+esc(ad.notes):''}</div>`:'<div class="card-note" style="margin-top:10px">尚未导入配置决策。可复制 Prompt 给 GPT 复核后导入 JSON。</div>'}<details style="margin-top:10px"><summary class="card-title" style="cursor:pointer">七个维度分析</summary><div style="margin-top:8px">${dims}</div></details><div class="modal-actions" style="justify-content:flex-start;margin-top:10px;flex-wrap:wrap"><button class="btn small" data-detail-action="copy-allocation-prompt">复制配置决策 Prompt</button><button class="btn ghost small" data-detail-action="import-allocation-json">导入配置决策 JSON</button></div></div>`;
}
function detailHeroPanel(s,mv,actual,deviation){
  ...
  return `<div class="card" style="margin-bottom:14px"><div class="entry-head"><div><div class="entry-name">${esc(s.name||'未命名标的')}</div><div class="entry-code">${esc(s.code||'无代码')} · ${esc(category)}</div></div><div class="chips"><span class="chip role">${esc(s.role||'—')}</span><span class="chip tag">${esc(s.theme||'—')}</span><button class="btn ghost small" id="backToListBtn" type="button">返回列表</button></div></div></div><div class="dash detail-first-screen"><div class="card"><div class="card-title">成本价</div><div class="card-num">${fmtMaybe(s.avgCost)}</div><div class="card-note">买入均价</div></div><div class="card"><div class="card-title">当前价</div><div class="card-num">${current}</div><div class="card-note">${currentDisplay(s)}</div></div><div class="card"><div class="card-title">持仓数量</div><div class="card-num">${fmtInt(s.shares)}</div><div class="card-note">目标 ${fmtMaybe(s.targetPct,1)}% · 实际 ${actual===null?'—':actual.toFixed(1)+'%'}</div></div><div class="card"><div class="card-title">当前市值</div><div class="card-num">${fmtMoney(mv)}</div><div class="card-note">仓位偏差 ${deviation}</div></div></div>`;
}
...
function technicalAnalysisPromptText(stock){
  ...
  return [
    ...
    '- 顶层只能是一个 JSON 对象，不要包在 technicalData、markdown 或代码块里。',
    '',
    '当前股票上下文：',
    JSON.stringify(ctx,null,2),
    '',
...
function fullAddDiscussionPromptText(stock){
  ...
  return [
    `你是一名谨慎的投资决策复核助手。`,
    '',
    `我想讨论是否可以用新增资金加仓【${stock.name||'未填写'}】。请不要直接给确定性买卖指令，只做条件化分析和加仓计划复核。`,
    '',
    '【股票信息】',
    `- 股票名称：${stock.name||'未填写'}`,
    ...
    '',
    '注意：JSON 必须使用英文半角双引号，不要使用中文弯引号；不要输出确定性买卖命令。'
  ].join('\n');
}
