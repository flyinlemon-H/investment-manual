...
function returnToActiveV13Review(){
  ...
  openV13DecisionReview(ctx.stockId,ctx.recId||'');
}
...
function openV13DecisionReview(stockId,recId){
  ...
  const price=cm.priceSnapshot&&cm.priceSnapshot.price!==null&&cm.priceSnapshot.price!==undefined?fmtMaybe(cm.priceSnapshot.price,2):(getComparablePrice(stock)||stockCurrentPrice(stock)||'暂无完整依据');
  body.innerHTML=`<div class="card" style="margin-bottom:12px"><div class="card-title">当前主要复核任务</div><div class="card-num" style="font-size:20px;white-space:normal">${esc(v13RecommendationTypeLabel(rec.type))}</div><div class="card-note">${esc(v13RecommendationPriorityLabel(rec.priority))} · ${esc(v13RecommendationTaskSummary(stock,rec))}</div><div class="card-note">来源 ${esc(v13RecommendationSourceLabel(rec.source))}${planName?` · 关联计划 ${esc(planName)}`:''}</div></div>`
    +v13DecisionReviewFactPanelHtml(stock,rec,plan,price)
    +`<div class="card" style="margin-bottom:12px"><div class="card-title">支持本次复核的依据</div><div class="card-note">以下内容只作为当前任务的支持依据，不作为第二张待处理任务。</div>${v13DecisionReviewSecondaryHtml(stock,rec)}</div>`
...
function planWorkspacePanel(stock){
  ...
  const refreshPanel=`<div class="card" style="margin-bottom:12px;background:rgba(255,255,255,.56)"><div class="card-title">刷新计划</div><div class="card-note">复制 Prompt，粘贴 AI 返回 JSON，导入后仍停留在当前详情页并刷新计划显示。</div><div class="modal-actions" style="justify-content:flex-start;margin-top:10px"><button class="btn small" type="button" data-detail-action="open-plan-refresh">刷新计划</button></div></div>`;
  const refreshInfo=`<div class="card" style="margin-bottom:12px;background:rgba(255,255,255,.56)"><div class="card-title">最近刷新</div><div class="card-note">刷新时间 ${esc(refresh.updated||'尚未刷新')} · 下次建议刷新 ${esc(refresh.validUntil||'尚未刷新')}</div></div>`;
  return `${v13StockEventPanel(stock)}${refreshPanel}${refreshInfo}<div class="card" style="margin-bottom:12px"><div class="card-title">当前有效计划</div>${v13PlanCenterSummary(stock)}</div><details class="card" style="margin-bottom:12px"><summary class="card-title" style="cursor:pointer">详细计划</summary><div style="margin-top:10px">${v13PlanCenterDetailSections(stock)}</div></details><details class="card" style="margin-bottom:12px"><summary class="card-title" style="cursor:pointer">历史计划</summary><div style="margin-top:10px">${v13PlanCenterHistory(stock)}</div></details>${positionPlanPanel(stock)}`;
}
function technicalWorkspacePanel(stock){
  return `${technicalAnalysisPanel(stock)}${trendRiskManagementPanel(stock)}${technicalSignalPanel(stock)}`;
}
function newsWorkspacePanel(stock){
  return `${shortTermCatalystPanel(stock)}${shortTermSentimentPanel(stock)}${informationCompletenessPanel(stock)}`;
}
...
function valuationAllocationWorkspacePanel(stock){
  ...
  return `${valuationAnalysisPanel(stock)}${valuationSignalPanel(stock)}${allocationDecisionPanel(stock)}${positionSummary}${positionManagementReviewPanel(stock)}`;
}
...
function longTermWorkspacePanel(stock){
  return `${longTermLogicPanel(stock)}${longTermMemoPanel(stock)}`;
}
function stockWorkspaceAccordion(stock){
  ...
  return `<div class="hint" style="margin-bottom:12px"><b>当前工作区：</b>${esc(activeTitle)}。一次只展开一个工作区，减少手机页面长度。</div>${stockWorkspaceSection('plan','Plan Workspace','当前事件、刷新计划、当前有效计划、详细计划、历史计划',planWorkspacePanel(stock),'var(--gold)')}${stockWorkspaceSection('technical','Technical Workspace','技术判断、风险、机会、依据、Prompt 与导入',technicalWorkspacePanel(stock),'var(--teal)')}${stockWorkspaceSection('news','News Workspace','新闻、催化、情绪资金、Prompt 与导入',newsWorkspacePanel(stock),'var(--seal)')}${stockWorkspaceSection('fundamental','Fundamental Workspace','财报复核、财务质量、盈利能力、现金流、基本面风险',fundamentalWorkspacePanel(stock),'var(--purple)')}${stockWorkspaceSection('valuation','Valuation / Allocation Workspace','估值复核、配置建议、仓位偏离、估值风险',valuationAllocationWorkspacePanel(stock),'var(--ink2)')}${stockWorkspaceSection('longterm','Long-term Workspace','长期逻辑、投资备忘录、长期风险',longTermWorkspacePanel(stock),'var(--ink)')}`;
}
