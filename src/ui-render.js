...
function calculateTechnicalDecision(stock){
  ...
  const seriousRisk=trendIsBreakdown||explicitBreakdown||(belowSupport&&(belowMa20||belowMa60));
...
function technicalAnalysisPanel(stock){
  const td=normalizeTechnicalData(stock.technicalData);
  const maText=`MA5 ${fmtTechnicalNumber(td.ma5,2)} · MA10 ${fmtTechnicalNumber(td.ma10,2)} · MA20 ${fmtTechnicalNumber(td.ma20,2)} · MA60 ${fmtTechnicalNumber(td.ma60,2)}`;
  const updated=td.priceUpdatedAt||td.lastUpdated||'—';
  const actionHint=formatChineseText(td.actionHint||'—');
  const summary=formatChineseText(td.technicalSummary||td.trendNote||'—');
  return `<div class="card" style="margin-bottom:14px"><div class="card-title">技术面分析</div><div class="dash" style="margin:0"><div><div class="card-title">当前价格</div><div class="card-num">${fmtMaybe(td.price!==null?td.price:stockCurrentPrice(stock),2)}</div><div class="card-note">${esc(td.timeframe||'daily')} · ${esc(updated)}</div></div><div><div class="card-title">均线状态</div><div class="card-note">${maText}</div></div><div><div class="card-title">成交量状态</div><div class="card-note">${esc(technicalVolumeStatus(td))}</div></div><div><div class="card-title">趋势判断</div><div class="card-note">${esc(zhTrendStatus(td.trendStatus)||'—')}</div></div></div><div class="text" style="max-width:none;margin-top:10px"><b>支撑位：</b>${technicalLevelList(td.supportLevels)}<br><b>压力位：</b>${technicalLevelList(td.resistanceLevels)}<br><b>技术面风险：</b>${td.riskFlags.length?zhList(td.riskFlags).map(esc).join('；'):'—'}<br><b>操作提示：</b>${esc(actionHint)}${englishTextHint(actionHint)}<br><b>摘要：</b>${esc(summary)}${englishTextHint(summary)}</div><div class="modal-actions" style="justify-content:flex-start;margin-top:10px;flex-wrap:wrap"><button class="btn ghost small" data-detail-action="copy-technical-prompt">复制技术面分析 Prompt</button><button class="btn ghost small" data-detail-action="import-technical-json">导入技术面 JSON</button></div></div>`;
}
...
function ensureSentimentImportModal(){
  ...
  el.innerHTML=`<div class="modal"><h2>导入情绪 Review JSON</h2><div class="modal-sub">支持 sentimentReview 对象或顶层包含 sentimentReview。导入后不会自动修改配置决策或当前操作建议。</div><div class="form-row"><label>粘贴 JSON</label><textarea id="sentimentImportText" style="min-height:280px;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:12px" placeholder='{\"sentimentReview\":{\"conclusion\":\"...\",\"importance\":\"high\"},\"sources\":[]}'></textarea></div><div class="modal-actions"><button class="btn ghost" id="sentimentImportCancelBtn" type="button">取消</button><button class="btn" id="sentimentImportSaveBtn" type="button">导入情绪 Review</button></div></div>`;
  document.body.appendChild(el);
...
function importSentimentReviewJson(){
  const stock=state.stocks.find(x=>x.id===detailStockId);
  if(!stock)return;
  const original={sentimentReview:JSON.parse(JSON.stringify(stock.sentimentReview||{})),dataFreshness:JSON.parse(JSON.stringify(stock.dataFreshness||{}))};
  let parsed;
  try{parsed=extractFirstJsonObject(document.getElementById('sentimentImportText').value)}catch(e){alert('导入失败：JSON 无法解析。');return}
  try{
    const payload=parsed&&parsed.sentimentReview?parsed.sentimentReview:parsed;
    if(!payload||typeof payload!=='object'||Array.isArray(payload))throw new Error('JSON 必须是 sentimentReview 对象，或顶层包含 sentimentReview。');
