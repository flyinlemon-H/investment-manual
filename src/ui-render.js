...
function positionPlanPanel(stock){
  ...
  const rows=activePlans.slice(0,4).map(p=>`${p.action==='sell'?'减仓':'加仓'} ${fmtInt(p.shares)} @ ${fmtMaybe(p.price)} · ${p.note||'—'}`);
  ...
  const buyRows=activePlans.filter(p=>(p.action||'buy')!=='sell').map(p=>`${fmtMaybe(p.price)}：${fmtInt(p.shares)} · ${p.note||'—'}`);
  const sellRows=activePlans.filter(p=>p.action==='sell').map(p=>`${fmtMaybe(p.price)}：${fmtInt(p.shares)} · ${p.note||'—'}`);
  ...
  const originalPlanContent=activePlans.length?`<details class="card" style="margin-top:10px" open><summary class="card-title" style="cursor:pointer">当前有效计划</summary><div class="text" style="max-width:none;margin-top:8px"><b>有效计划：</b>${planList(allActiveRows)}<b>加仓/观察：</b>${planList(buyRows)}<b>减仓计划：</b>${planList(sellRows)}</div></details>`:'<div class="card-note">暂无当前有效计划。</div>';
...
function planListHtml(plans,type,cp){
  const arr=v13DisplayActivePlans(plans).filter(p=>type==='buy'?(p.action||'buy')==='buy':p.action==='sell').sort((a,b)=>Number(b.price||0)-Number(a.price||0));
