...
function stockWorkspaceAccordion(stock){
  ...
  return `<div class="hint" style="margin-bottom:12px"><b>当前工作区：</b>${esc(activeTitle)}。一次只展开一个工作区，默认显示摘要，详情可展开。</div>${stockWorkspaceSection('plan','计划工作区','刷新计划、当前事件摘要、当前有效计划摘要',planWorkspacePanel(stock),'var(--gold)')}${stockWorkspaceSection('technical','技术面工作区','技术判断、风险、机会摘要',technicalWorkspacePanel(stock),'var(--teal)')}${stockWorkspaceSection('news','新闻催化工作区','新闻、催化、情绪资金摘要',newsWorkspacePanel(stock),'var(--seal)')}${stockWorkspaceSection('fundamental','基本面工作区','财报复核、财务质量、现金流摘要',fundamentalWorkspacePanel(stock),'var(--purple)')}${stockWorkspaceSection('valuation','估值/配置工作区','估值复核、配置建议、仓位偏离摘要',valuationAllocationWorkspacePanel(stock),'var(--ink2)')}${stockWorkspaceSection('longterm','长期逻辑工作区','长期逻辑、投资备忘录、长期风险摘要',longTermWorkspacePanel(stock),'var(--ink)')}`;
}
