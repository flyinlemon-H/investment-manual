...
function saveState(candidateState,options={}){
  const hasCandidate=arguments.length>0&&candidateState!==undefined&&candidateState!==null;
  const nextState=normalize(hasCandidate?candidateState:state);
  nextState.updatedAt=Date.now();
  localStorage.setItem(STORAGE_KEY,JSON.stringify(nextState));
  if(!hasCandidate||options.critical!==true)state=nextState;
  return nextState;
}
